...
def config():
    ...
    parser.add_argument("--max_dataset_size", type=int, default=3)
    parser.add_argument(
        "--concept_category",
        type=str,
        default="language_translation",
...
def linearity_token():
    ...
    layers = model.config.num_hidden_layers
    logger.info(f"Measuring linearity for {layers} layers")
    concept_vector = concept_vectors[3, :]
    progress_bar = tqdm(total=args.max_dataset_size, desc="Measuring linearity")
    each_token_lss = []
    idy = 0
    # diff_list = []
...
def linearity_each_layer():
    ...
    # We steer at layer 2
    steer_layer = 12
    concept_vector = concept_vectors[steer_layer, :]
    ...
    layers_to_measure = list(range(steer_layer + 1, num_layers, 3))
